...
const UserDetails = () => {
  ...
  const { userId } = useParams(); //retrieving user ID from URL parameters.
  const [user, setUser] = useState(null); //state which stores user details
  const [roles, setRoles] = useState([]); //state which stores the role of the user in the system
  const [selectedRole, setSelectedRole] = useState("");
  const [error, setError] = useState(null);
  const [isEditingPassword, setIsEditingPassword] = useState(false);

  const fetchUserDetails = useCallback(async () => {
    //fetches all the user specific details that is represented on user:id page for admins.
    setLoading(true);
    try {
  ...
  useEffect(() => {
    ...
    if (user && Object.keys(user).length > 0) {
      //setting up the values in form that is retrieved from user details through backend API call.
  ...
  const handleUpdateRole = async () => {
    ...
    try {
      //creating an object with updated data regarding user role.
      ...
      }); //sends the request to update the role
      fetchUserDetails();//refresh the data with the updated changes
